...
###########################################
echo "bfaptitudeinstallvim01a.sh :            Installation et configuration de vim (Partie 01a)"
...
if [ ! -e /var/log/bf/bfaptitudeinstallvim01-vimtiny.log ]
then
  ...
  date +"%F %T" >> /var/log/bf/bfaptitudeinstallvim01-vimtiny.log
fi

# installation des paquets debian
if [ ! -e /var/log/bf/bfaptitudeinstallvim01-vimnox.log ]
then
  ...
  echo y | aptitude install vim-common 1> /dev/null
  echo y | aptitude install vim-runtime 1> /dev/null
  echo y | aptitude install vim-nox 1> /dev/null
  ...
  # hash -rf
  date +"%F %T" >> /var/log/bf/bfaptitudeinstallvim01-vimnox.log
fi
...
then
  ...
  cp /etc/vim/colors/herald.vim /etc/vim/colors/bfherald.vim
  sed -i -e 's/hi CursorColumn   ctermbg=0/hi CursorColumn   ctermbg=4/g' \
      /etc/vim/colors/bfherald.vim
  sed -i -e 's/hi CursorLine     ctermbg=0/hi CursorLine     ctermbg=4/g' \
      /etc/vim/colors/bfherald.vim
  cd $OLDPWD
  date +"%F %T" >> /var/log/bf/bfaptitudeinstallvim01-colorscheme.log
fi

# installation des dictionnaires de langue
if [ ! -e /var/log/bf/bfaptitudeinstallvim01-lang.log ]
then
  ...
  cd $OLDPWD
  date +"%F %T" >> /var/log/bf/bfaptitudeinstallvim01-lang.log
fi

# installation du gestionnaire d'extension vim vundle
#if [ ! -e /var/log/bf/bfaptitudeinstallvim01-vundle01.log ]
#then
...
#  cd $OLDPWD
#  date +"%F %T" >> /var/log/bf/bfaptitudeinstallvim01-vundle01.log
#fi
...
#if [ ! -e /var/log/bf/bfaptitudeinstallvim01-vimpager.log ]
#then
...
#
#  date +"%F %T" >> /var/log/bf/bfaptitudeinstallvim01-vimpager.log
#fi
