...
echo "bfaptitudeclean-preseed.sh :                    Nettoyage 2"

if [ ! -e /var/log/bf/bfaptitudeclean-preseed-clean.log ]
then
  echo "  Aptitude clean"
  echo y | aptitude clean 1> /dev/null 2> /dev/null
  date +"%F %T" >> /var/log/bf/bfaptitudeclean-preseed-clean.log
fi

if [ ! -e /var/log/bf/bfaptitudeclean-preseed-purge.log ]
then
  echo "  Aptitude purge"
  echo y | aptitude purge "~c" 1> /dev/null 2> /dev/null
  date +"%F %T" >> /var/log/bf/bfaptitudeclean-preseed-purge.log
fi
...
if [ ! -e /var/log/bf/bfaptitudeclean-preseed-alternatives.log ]
then
  echo "  Alternatives"
  ...
  date +"%F %T" >> /var/log/bf/bfaptitudeclean-preseed-alternatives.log
fi
